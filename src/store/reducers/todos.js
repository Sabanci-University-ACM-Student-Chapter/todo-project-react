// A reducer to store Todos information.

let todos = JSON.parse(localStorage.getItem("Todos"))

if(todos === null){ // creates tutorial todos
...
const TodoReducer = (state=todos,action) => {
    switch(action.type){
        case 'CHANGE_COMPLETED': // when you click completed for a todo, this part works and changes todo's completion
            let todo = todos[action.self_id] || todos[0]
            todo.isCompleted = !todo.isCompleted
            localStorage.setItem("Todos", JSON.stringify(todos))
            todos = JSON.parse(localStorage.getItem("Todos"))
            return todos
        case 'UPDATE_DETAIL': // It enables to 'Edit Mode' for details.
            let todo2 = todos[action.self_id]
            todo2.detail = action.text
            return todos
        case 'SAVE_DETAIL': // saves your changes in details.
            localStorage.setItem("Todos", JSON.stringify(todos))
            return todos
        case 'CANCEL_DETAIL': // cancels your changes in details.
            todos = JSON.parse(localStorage.getItem("Todos"))
            return todos
        case 'REMOVE_TODO': // removes a todo, then synchronizes ids to true index
            ...
            todos = JSON.parse(localStorage.getItem("Todos"))
            return rest2
        case 'ADD_TODO': // its add a new todo to application
            let todos3 = [...action.todoList, action.newTodo]
            localStorage.setItem("Todos", JSON.stringify(todos3))
            todos = JSON.parse(localStorage.getItem("Todos"))
            return todos3
        case 'CHANGE_LABEL': // it works to change todos' labels.
